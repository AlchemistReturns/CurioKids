...
const ChildRegistrationScreen = () => {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (parentSnap.empty) {
        throw new Error('Invalid parent key. Please check and try again.');
      }
      ...
      const message =
        err?.message ?? err?.code
          ? String(err.message || err.code).replace('Firebase: Error (auth/', '').replace(').', '').replace(/-/g, ' ')
  ...
  return (
    <View className="flex-1 justify-center px-6">
      <Text className="text-3xl font-bold mb-8 text-center text-primary">Hey Kid!</Text>
      ...
      {error ? <Text className="text-primary mb-4 text-center">{error}</Text> : null}
