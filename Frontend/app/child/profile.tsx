    ...
    <SafeAreaView className="flex-1 bg-base" edges={['top']}>
      <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
        ...
        <View className="items-center mt-8 mb-10">
          <View className="h-24 w-24 bg-primary rounded-full justify-center items-center mb-4">
          ...
          <Text className="text-primary font-medium">{user.email}</Text>
        </View>

        {/* Academic Performance Section */}
        <Text className="text-primary text-xl font-bold mb-4">Subject Points</Text>

        {subjectKeys.length === 0 ? (
          <View className="bg-primary p-6 rounded-2xl items-center mb-8">
            <Ionicons name="book-outline" size={48} color="#fff" className="mb-2 opacity-50" />
            <Text className="text-white text-center">No marks recorded yet.</Text>
            <Text className="text-white/60 text-xs mt-1">Play games to earn scores!</Text>
          </View>
        ) : (
          <View className="mb-8">
            {subjectKeys.map((subject, index) => (
              <View
                key={index}
                className="flex-row items-center justify-between bg-primary p-5 rounded-2xl mb-3 shadow-sm"
              >
                <View className="flex-row items-center">
                  <View className="bg-base p-2 rounded-lg mr-4">
                    <Ionicons name="bookmark" size={20} color="#3f51b5" />
                  </View>
        ...
        {/* Account Actions */}
        <View className="mb-8">
          <TouchableOpacity className="flex-row items-center justify-between bg-primary p-4 rounded-xl mb-3">
            <View className="flex-row items-center">
